...
const BANK_COLORS: Record<string, string> = {
  ...
  c6: '#242424', sicoob: '#007A3D', auto: colors.green, outro: colors.t3,
};
...
export default function UploadScreen({ navigation }: any) {
  ...
  const [slots, setSlots] = useState<Slot[]>([{ id: 1, bank: 'auto', detecting: false, file: null }]);
  ...
  const pickFile = async (slotId: number) => {
    try {
      ...
      // Marca como detectando
      setSlots(prev => prev.map(s => s.id === slotId ? { ...s, detecting: true, file: { uri, name, ext } } : s));

      // Auto-detecta banco
      const detected = await detectBank(uri, name);
      setSlots(prev => prev.map(s => s.id === slotId ? { ...s, detecting: false, bank: detected } : s));

      // Adiciona slot vazio se não existir
      setSlots(prev => {
        const hasEmpty = prev.some(s => !s.file && s.id !== slotId);
        if (!hasEmpty && prev.length < 5) {
          return [...prev, { id: Date.now(), bank: 'auto', detecting: false, file: null }];
        }
        return prev;
      });
    } catch {
  ...
  const removeSlot = (slotId: number) => {
    setSlots(prev => {
      ...
      // sempre deixa pelo menos 1 slot vazio
      const hasEmpty = filtered.some(s => !s.file);
      if (!hasEmpty) return [...filtered, { id: Date.now(), bank: 'auto', detecting: false, file: null }];
      return filtered;
    });
  };
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      for (const slot of filled) {
        ...
        const bankLabel = BANKS.find(b => b.id === slot.bank)?.label || slot.bank;
        slotResults.push({ slotId: slot.id, bankLabel, txs });
      }

      setProcessStep('Detectando transferências...');
      const transferKeys = detectTransfersMulti(slotResults);

      // Detectar mês pelo arquivo
  ...
  const bankLabel = (id: string) => BANKS.find(b => b.id === id)?.label || 'Outro';
  const bankColor = (id: string) => BANK_COLORS[id] || colors.t3;
  ...
  return (
    <View style={s.root}>
        ...
        <View style={s.card}>
          ...
          {filledSlots.map(slot => (
            <View key={slot.id} style={s.fileRow}>
              ...
              {/* Badge do banco */}
              <TouchableOpacity
                style={[s.bankBadge, { backgroundColor: bankColor(slot.bank) }]}
                onPress={() => setBankModal(slot.id)}
              >
                {slot.detecting
                  ? <ActivityIndicator size="small" color="#fff" />
                  : <Text style={s.bankBadgeText}>{bankLabel(slot.bank)}</Text>
                }
              </TouchableOpacity>
            </View>
          ))}

          {/* Slot vazio — "Adicionar extrato" */}
          {emptySlot && (
      ...
      <Modal visible={bankModal !== null} transparent animationType="slide">
        <TouchableOpacity style={s.modalOverlay} activeOpacity={1} onPress={() => setBankModal(null)}>
          <View style={s.modalSheet}>
            <View style={s.modalHandle} />
            <Text style={s.modalTitle}>Selecionar banco</Text>
            {BANKS.map(b => {
              const active = slots.find(sl => sl.id === bankModal)?.bank === b.id;
              return (
                <TouchableOpacity key={b.id} style={[s.bankOpt, active && s.bankOptActive]} onPress={() => {
                  setSlots(prev => prev.map(sl => sl.id === bankModal ? { ...sl, bank: b.id } : sl));
                  setBankModal(null);
                }}>
                  <View style={[s.bankDot, { backgroundColor: BANK_COLORS[b.id] || colors.t3 }]} />
...
const s = StyleSheet.create({
  ...
  modalTitle:     { fontSize: 15, fontWeight: '600', color: colors.t1, marginBottom: 4 },
  bankOpt:        { flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: colors.s2, borderRadius: colors.rs, padding: 12, borderWidth: 0.5, borderColor: colors.b2 },
